...
Marks will be awarded based on the correct usage of the specified argument type in each menu option
  ...
  """ 
import question3
from question3 import addprofile,addmultiple,customerprofile,geninvoice,product 
while True:
    ...
    match ch:
        case 1:
            ...
            addprofile.profile(name,email,mb)
            print("registration done successfully")
        case 2:
            pname=input("enter product name: ")
            pp=int(input("enter product price: "))
            cat=input("enter category")
            product.pro(product_name=pname,product_price=pp,category=cat)
        case 3:
            pro_name=input("enter product name: ")
            pro_price=float(input("enter product price: "))
            geninvoice.genin(pro_name,pro_price)
        case 4:
            m=[]
            n=int(input("enter noof prouct: "))
            for i in range(n):
                x=int(input("enter product price: "))
                m.append(x)
            print("total price",addmultiple.multiple(m))
        case 5:
            ...
            customerprofile.profile(name=name,city=city,email=email,mobile=mobile,member=member)
        case 6:
            print("exit")
            break
